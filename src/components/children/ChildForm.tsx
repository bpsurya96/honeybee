  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (isEditing && initialData) {
        // Use API route for update (validates ownership server-side)
        const res = await fetch(`/api/children/${initialData.id}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ name: name.trim(), date_of_birth: dateOfBirth, gender: gender || null }),
        })
        const json = await res.json()
        if (!res.ok) { setError(json.error || 'Update failed.'); return }
      } else {
        // Direct Supabase insert for create (RLS protects ownership)
    ...
    <div className="bg-white rounded-3xl p-6 shadow-sm border border-stone-100">
      <form onSubmit={handleSubmit} className="space-y-5" noValidate>
        ...
        <div>
          ...
          <input
            id="childName" type="text" required value={name} onChange={(e) => setName(e.target.value)}
        ...
        <div>
          <label htmlFor="dateOfBirth" className="block text-sm font-semibold text-stone-700 mb-1.5">
            Date of birth <span className="text-red-500">*</span>
          </label>
          <input
            id="dateOfBirth" type="date" required value={dateOfBirth}
            max={new Date().toISOString().split('T')[0]} onChange={(e) => setDateOfBirth(e.target.value)}
            className="w-full px-4 py-3 rounded-2xl border border-stone-200 focus:border-amber-400 focus:outline-none focus:ring-2 focus:ring-amber-200 transition-all text-stone-900 bg-white"
          />
        </div>
        <div>
        ...
        <div className="flex gap-3 pt-2">
          <button type="button" onClick={() => router.back()} className="flex-1 border-2 border-stone-200 text-stone-600 font-semibold py-3 rounded-full hover:bg-stone-50 transition-colors">
            Cancel
          </button>
          <button type="submit" disabled={loading} className="flex-1 bg-amber-500 hover:bg-amber-600 disabled:bg-amber-300 text-white font-bold py-3 rounded-full transition-all shadow-md disabled:cursor-not-allowed">
            {loading ? (isEditing ? 'Saving...' : 'Adding...') : (isEditing ? 'Save Changes' : 'Add Child')}
          </button>
        </div>
      </form>
    </div>
